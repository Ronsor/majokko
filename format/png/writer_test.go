...
func encodeDecodeZstd(m image.Image) (image.Image, error) {
	var b bytes.Buffer
	enc := &Encoder{UseZstd: true}
...
func TestZstdWriter(t *testing.T) {
	// The filenames variable is declared in reader_test.go.
	names := filenames
	if testing.Short() {
		names = filenamesShort
...
func TestWriterLevels(t *testing.T) {
	...
	if b2.Len() <= b1.Len() {
		t.Errorf("DefaultCompression encoding was larger than NoCompression encoding: %d vs %d", b2.Len(), b1.Len())
	}
