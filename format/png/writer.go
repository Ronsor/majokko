...
func (e *encoder) writeImage(w io.Writer, m image.Image, cb int, level int) error {
	...
	if !e.useZstd {
		...
		if e.zs == nil || int(e.zsLevel) != level {
			zs, err := zstd.NewWriter(w, zstd.WithEncoderLevel(zstd.EncoderLevel(level)))
			if err != nil {
				return err
			}
			e.zs = zs
			e.zsLevel = zstd.EncoderLevel(level)
		} else {
			e.zs.Reset(w)
		}
	...
	for y := b.Min.Y; y < b.Max.Y; y++ {
		...
		if _, err := cw.Write(cr[f]); err != nil {
			return err
		}
...
func (e *encoder) writeIDATorZDATs() {
	if e.err != nil {
		return
	}
	if e.bw == nil {
...
// This function is required because we want the zero value of
// Encoder.CompressionLevel to map to zstd.SpeedDefault.
func levelToZstd(l CompressionLevel) int {
	switch l {
	case DefaultCompression:
...
// Chunk is a PNG chunk.
type Chunk struct {
	// Name is the 4-byte chunk ID.
	Name string
	// Data is the chunk payload.
	Data []byte
	// AfterIDAT specifies whether the chunk should be placed
	// before or after the image data.
	AfterIDAT bool
}
...
func (enc *Encoder) Encode(w io.Writer, m image.Image) error {
	return enc.EncodeWithOptions(w, m, &EncodeOptions{})
}

// EncodeWithOptions writes the Image m to w in PNG format, taking
// into account the extra options specified.
