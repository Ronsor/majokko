...
type decoder struct {
	...
	// unknownChunkCb is called when an unrecognized chunk is read
	unknownChunkCb func (Chunk) error
}
...
// decodeZDAT decodes the ZDAT data into an image.
func (d *decoder) decodeZDAT() (image.Image, error) {
	r, err := zstd.NewReader(io.LimitReader(d, int64(d.idatLength)))
	if err != nil {
		return nil, err
	}
	defer r.Close()
...
func (d *decoder) parseZDAT(length uint32) (err error) {
	d.idatLength = length
	d.img, err = d.decodeZDAT()
	if err != nil {
		return err
	}
	return d.verifyChecksum()
...
func (d *decoder) parseChunk() error {
	...
	if d.unknownChunkCb == nil {
		...
		d.crc.Write(data)
		err = d.unknownChunkCb(Chunk{
			Name: string(d.tmp[4:8]),
			Data: data,
			AfterIDAT: d.stage > dsSeenIDAT,
		})
		if err != nil {
			return err
		}
...
// DecodeOptions are additional options that can be passed to the PNG
// decoder.
type DecodeOptions struct {
	// ParseUnknownChunk specifies a function to call when an
	// unrecognized PNG chunk is encountered.
	ParseUnknownChunk func (Chunk) error
}
...
// DecodeWithOptions reads a PNG image from r and returns it as an
// image.Image. The type of Image returned depends on the PNG contents.
// Additional options can be passed to the decoder as DecodeOptions.
func DecodeWithOptions(r io.Reader, o *DecodeOptions) (image.Image, error) {
	d := &decoder{
		r:   r,
		crc: crc32.NewIEEE(),
		unknownChunkCb: o.ParseUnknownChunk,
	}
